...
void heapify (heap_t *heap, int i) {
  ...
  if ((left <= heap->size) && 
      ((heap->item[left-1].key > heap->item[i-1].key) ||
       ((heap->item[left-1].key == heap->item[i-1].key) &&
	(heap->item[left-1].secondary_key < heap->item[i-1].secondary_key)))) 
    largest = left;
  else
    largest = i;
  ...
  if ((right <= heap->size) && 
      ((heap->item[right-1].key > heap->item[i-1].key) ||
       ((heap->item[right-1].key == heap->item[i-1].key) &&
	(heap->item[right-1].secondary_key < heap->item[i-1].secondary_key)))) 
    largest = right;
...
heap_t *heap_insert (heap_t *heap, heap_item *item) {
  ...
  while ((i > 1) && 
	 ((heap->item[PARENT(i)-1].key < item->key) ||
	  ((heap->item[PARENT(i)-1].key == item->key) &&
	   (heap->item[PARENT(i)-1].secondary_key > item->secondary_key)))) { 
