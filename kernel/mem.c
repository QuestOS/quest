...
static void pow2_split_block(uint8 index) {
  uint8 *ptr1, *ptr2;
...
static void pow2_insert_used_table(uint8 *ptr, uint8 index) {
...
static int pow2_remove_used_table(uint8 *ptr, uint8 *index) {
  int i;
...
static uint8 pow2_compute_index(uint16 size) {
  int i;
