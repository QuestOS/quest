...
BOOL sb_dsp_detect_irq_number (WORD base_address, BYTE *irq_number) {
	...
	case 1: dsp_irq_number = 2; break;  /* IRQ 2 */
	case 2: dsp_irq_number = 5; break;  /* IRQ 5 */
	case 4: dsp_irq_number = 7; break;  /* IRQ 7 */
	case 8: dsp_irq_number = 10; break; /* IRQ 10 */
	default:dsp_irq_number = 0;         /* Error */
    }
...
PRIVATE	void driver_setup_dma_transfer (int memory_size) {
  ...
  outb ((dma_buffer_phys_base>>16)&0xFF, driver_dma_page[dsp_dma_channel_8]);
...
PRIVATE	void driver_setup_dsp_transfer (int memory_size) {
...
static void play_sample( void ) {
    ...
    memcpy( (void *) dma_buffer_virt_base, filebuffer + fileoffset,
	    chunksize );
