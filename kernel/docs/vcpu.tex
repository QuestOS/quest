...
time $V_a$.  Assume $\text{IDLE}_T=\infty$.  IO-VCPUs have eligibility
time $V_e$, utilization $V_U$, and a notion of status being one of:
incomplete, complete, or budgeted.

Algorithm~\ref{alg:vcpuschedule} is the scheduler, invoked
periodically or to switch to the next runnable VCPU.
Algorithm~\ref{alg:iovcpuwakeup} is meant to be used when waking IO
tasks (such as bottom halves) that are doing work on behalf of another
VCPU.  Algorithm~\ref{alg:iovcpucomplete} is invoked by an IO task
when it is finished with the job.

\begin{algorithm}
  \caption{\tt schedule}\label{alg:vcpuschedule}
  \begin{algorithmic}[1]
    ...
    \IF{$V$ is IO-VCPU}
    \IF{$V$ completed job \OR exhausted budget}
    \IF{$V_R=\emptyset$}
    \STATE Set $V_e:=V_e+u/V_U$ where $u$ is the amount of budget consumed.
    \STATE Post replenishment event on VCPU $V$ for $V_T\cdot V_U$ budget at time $V_e$.
    \ENDIF
    \IF{$V$ completed job}\STATE Set $V$ to status ``incomplete''.\ENDIF
    \ENDIF
    \ELSE
    \STATE Post replenishment event on VCPU $V$ for $\Delta t$ budget at time $V_a+V_T$.

    \ENDIF
    \FOR[{Apply replenishments}]{each VCPU $v\in\bar V$}
    \FOR{each replenishment $(t,b)\in v_R$}
    \IF{$t\leq t_{cur}$}
    \STATE add $b$ to $v_b$
    \IF{$v$ is \NOT IO-VCPU}\STATE set $v_a:=t_{cur}$.\ENDIF
    \ENDIF
    \ENDFOR
    \ENDFOR
    ...
    \IF{there is no satisfactory $V_{next}$} \STATE enter idle mode
    and go to step~\ref{sched:repltimer}.\ENDIF
    \STATE Internally schedule $V_{next}$.
    \IF{$V_{next}$ has empty runqueue}\STATE let $\bar V'=\bar
    ...
    \FOR{each replenishment $(t,b)\in v_R$}
    \IF{$t < \Delta t'$}\STATE set $\Delta t':= t - t_{cur}$\ENDIF
    \ENDFOR
    \ENDFOR
    \FOR[Check for activations]{every non-IO VCPU $v$ on this PCPU}
    \IF{$v_b>0$ \AND $T_{next}\leq v_T < T_{prev}$}\STATE set $v_a:=t_{cur}$.\ENDIF
    \ENDFOR
...
\begin{algorithm}
  \caption{\tt IO-VCPU-wakeup}\label{alg:iovcpuwakeup}
  \begin{algorithmic}[1]
    \REQUIRE Handle of IO task $\tau$ to be woken.
    \REQUIRE Main VCPU $M$ associated with IO task.
    \REQUIRE $t_{cur}$ is current time.
    \STATE Invoke {\tt wakeup} on $\tau$
    \STATE Let $V$ be the IO-VCPU $V_{\tau}$ of task $\tau$.
    ...
    \ENDIF
    \STATE Let $C_{max}$ be $V_T\cdot V_U$.
    \IF{$V_R=\emptyset$}
    \IF{$V$ has not been budgeted}
    \STATE Post replenishment on $V$ for $C_{max}$ budget at time $V_e$.
    \ENDIF
    \ELSE
    \STATE There is $V_R=\{r\}$, set $r_b:=C_{max}$.
    \ENDIF
    \STATE $V$ now has status ``budgeted''.
    \STATE Apply any pending replenishments on $V$.
...
\begin{algorithm}
  \caption{\tt IO-VCPU-job-complete}\label{alg:iovcpucomplete}
  \begin{algorithmic}[1]
    \STATE Set IO-VCPU to status ``complete''.
    \STATE Invoke scheduler.
  \end{algorithmic}
\end{algorithm}
\end{document}
